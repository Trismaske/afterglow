...
| D8 | State-editor action scope | **All four actions addable and removable** (organize add opens the album picker; share routes through the share-queue writers) | "The four actions align" — organize's target is a parameter, not an inherent inability. Refusals stay the honest three: trashed (the OS's), an applied organize, a resolved share. |
...
- **Four action rows**, each addable and removable: edit (queue / complete / remove via `leaveQueue`), favourite (directional, applied is reversible via target `'0'` — the heart-off badge already models it), share (through `addToShareQueue`/`removeFromShareQueue` — cycle semantics live there), organize (add opens the `AlbumPicker`; a queued move is removable).
