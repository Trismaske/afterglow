...
**m0.8.4 — drop Android ≤ 10 (decided 2026-07-30; next)**
- **Raise the floor to Android 11** (`minSdkVersion` 30) and delete every legacy branch: the pre-API-30 album-catalog fallback, the merged-collection URI shape, the API 24-28 mounted-volume arm, and the version gates that guard them. Below Android 11 there is no system trash — so culling, the product's core loop, has never worked there, while the app would still install; the floor makes that honest and the platform enforces it (sideloads refuse, Play hides the listing). Tristan owns no such device, so the support was untestable. Design doc settles the exact floor and per-module gate calls.
