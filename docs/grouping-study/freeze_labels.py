...
}, open('labels-v1.json', 'w'), indent=1)
...
for name in needed:
    if name in by_base:
        ...
        missing.append(name)
if missing:
    sys.exit(f'missing embeddings for {missing} — re-run embed.py first')
json.dump({'version': 'labels-v1', 'dim': 1280, 'encoding': 'base64 float32 LE, L2-normalized',
           'model_sha256': model_sha, 'photos': fix}, open('embeddings-labeled-v1.json', 'w'))
