...
115. **Refresh livelock fix (device-caught, post-round-44)**: the v14 S23 validation showed an EMPTY review queue for the entire 27 k warm rescan while the S10e's post-scan queue was fully populated — the round-35 retry-until-latest `refresh()` loops livelocked: concurrent scan-status refreshes (every ~2 s during a scan) mutually superseded each other's generations, so no pass ever committed until the scan settled. Fix: `refresh()` is now SINGLE-FLIGHT and COALESCING (one chain runs passes back-to-back; requests arriving mid-pass set a rerun flag and await the chain), and `refreshScoped` drains then HOLDS the same slot for its strict pass, serving coalesced requests with one follow-up pass after it settles. Barrier semantics are preserved (a caller's promise resolves only after a pass that started at/after its request committed or deliberately skipped); the generation guard stays as the commit-ordering backstop.
