...
Videos enter review (deferred from m0.7 by decision, singles-first: playback, keep/cull/queues; grouping later if warranted), plus whatever the m0.7 round surfaces — first item in (tester, 2026-07-23, on the 0.7.0 RC): **Home navigation redesign** — the queue cards crowd Home and push session start below the fold; proposal is a bottom navigation bar with an icon per queue (count-badged) around a bigger central Home/Review action so a session starts without scrolling (design pass needed: Material bottom nav caps at ~5 destinations vs 6 queue screens, and the bar should not steal deck space) — dormant shared-core debt (mix-engine weight contract, legacy `CullSession` hardening, retrospective selector validation), and **Android ≤ 10 culling via permanent delete** behind unmissable warnings (policy change 2026-07-23 — no system trash below API 30; ships with the matching trash-invariant doc updates). **After m0.8**: hardening and tester-driven fixes to 1.0 — including the planned **one-time signing break to a real release keystore** (testers reinstall; a data export/import path is considered first so review history survives) and merging `initial` → `main` as the pre-1.0 era closes. **iOS evaluation is deferred post-1.0 until further notice** (no iOS users or testers today).
