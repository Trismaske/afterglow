 ...
 * the user already worked through), when its group carries
 ...
 * member is still 'unreviewed' — or (m0.8.3 phase 2) when its group
 * holds an UNREACHABLE member: a pass while that member's volume is out
 * can only see the group's mounted side, and rebuilding it would strand
 * the unreachable photo in a rump group the <2-present repair then
 * dissolves — an assignment change plan §5 forbids ("unreachable photos
 * keep every row byte-for-byte … group membership"). The group unfreezes
 * the moment every member's volume is back.
 *
 ...
 * touched, so plan §5 byte-for-byte holds; groups frozen for review
 * reasons (verdicts, metadata, user singles) never grow.
 */
...
export function windowFreeze(windowIds: readonly string[], maps: ReconcileMaps): WindowFreeze {
  ...
  for (const id of windowIds) {
    ...
    if (members.some((member) => !unreviewed(member))) {
      frozen.add(id);
      continue;
    }
    ...
    // is the ONE freeze reason that leaves the group growable.
