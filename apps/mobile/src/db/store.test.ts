...
describe('getLifetimeStats', () => {
  it('returns unique-photo event totals plus successful reclaimed bytes', async () => {
    const getFirstAsync = vi
      .fn()
      .mockResolvedValueOnce({ reviewed: 31, culled: 8, editsCompleted: 5, favouritesApplied: 7 })
      .mockResolvedValueOnce({ n: 12_345 });
  ...
  it('normalizes an empty database to zeroes', async () => {
    const db = {
      getFirstAsync: vi.fn().mockResolvedValueOnce(null).mockResolvedValueOnce({ n: null }),
