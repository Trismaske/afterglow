...
| RAW strategy | Keep it — it's the differentiator. Per-image routing to the editor that owns the edits (sidecar + namespace sniffing): **darktable XMPs via `darktable-cli`**, **RawTherapee pp3s via `rawtherapee-cli`**, **Lightroom via the Afterglow companion plugin** driving the machine's own LR Classic (no headless Adobe path exists — verified). The rendering predicate: **render the RAW unless the RAW cannot reproduce what an existing JPEG already contains** — a JPEG sibling displays only when it carries something no render can recreate (lost edits, camera processing), always labeled; otherwise the show renders and the JPEG becomes redundant-export inventory for the organizer era. Lone unedited RAWs show the embedded camera preview when displaying it needs no upscale, else render (one toggle). JPEG-only libraries work fine regardless. |
...
| Mobile workflow | Three-layer photo state model — verdict · actions · annotations (see below); swipe-deck group review (replaced the m0.1–m0.3 duel bracket in m0.4; the bracket left core in m0.8 — desktop builds its own v0.7 compare model); stored compare/duel history instead of full ranking; completed groups advance directly to the next unfinished group; to-edit queue lives in-app with `ACTION_EDIT` launch. |
...
3. **Lightroom users — the companion plugin** (`apps/lightroom-plugin/`, hand-rolled Lua, file-based job queue): drives the machine's own LR Classic against a **dedicated Afterglow render catalog** — import, forced sidecar read (`photo:readMetadata()` — import alone loads no edits), `LrExportSession` into Afterglow's render buffer. User catalogs are never imported into or metadata-read. While a user's own LR session is open, jobs defer by default; the opt-in **close-Lightroom-on-screensaver-start** setting adds one exception to the close policy: on a screensaver-triggered launch only (manual starts never qualify) a user-owned session may be closed **gracefully** — a blocked close (unsaved dialog) falls back to deferral, and force-kill never happens. Outside that opt-in, Afterglow closes only instances it launched. Sidecars (`crs:` XMP + `.acr` AI masks) are machine-portable and render faithfully. Requires LR installed + running; Afterglow launches LR minimized when LR jobs are pending and gracefully closes only instances it launched. **Unattended operation is dialog-free**: both LR prompts — metadata-read and AI-update — are suppressed via preference keys verified/pre-seeded at onboarding (spike-verified on fresh AI-mask imports); AI-mask edits render faithfully from RAW + XMP alone, the `.acr` contributing nothing to rendered pixels.
4. **No editor / no edits:** the embedded camera preview (full-res for NEF/CR2; ~1080p for CR3/ARW), labeled "camera preview" — shown when displaying it needs no upscale, rendered otherwise (one toggle, default render-on-upscale).
5. **Anyone:** exported JPEGs always work, but a JPEG sibling (same folder or the `JPEG\` subfolder convention) is never a display shortcut when its RAW is renderable — the show renders, and the sibling is flagged at scan time as redundant-export inventory for the organizer era (behind a default-on tracking setting). Siblings display, labeled, exactly when they carry something no render can reproduce: a no-sidecar pair (the edits live only in the JPEG, or it carries the camera's own processing) or an unordered **multi-editor conflict** (an ordered conflict renders the newest sidecar's editor, badged and flagged for review; a sibling cannot be attributed to either editor, so it is withheld from the inventory).
...
- RAW via the tiers above, rendered just-in-time for a moment-aware look-ahead queue into an ephemeral buffer (deleted after leaving recent history; re-rendered on next appearance — compute is free on an idle machine). Render time is machine-dependent and unbounded by design: substitution first, then a hold with an honest render-wait status after an interval-scaled grace period (stated in settings) — never an error, never silently degraded quality.
...
- Flag-to-queue: single keypress while watching — **D**elete, **E**dit, **M**ove/misfiled, **R**eview, re**N**ame, da**T**e-fix — with an unobtrusive confirmation toast; slideshow never stops. Queue persists across sessions; automatic entries (the export inventory, multi-editor conflicts) carry their provenance and are counted and filtered separately from manual flags.
...
- Culling assistant: surface visually-similar groups via the shared embedding grouping engine (m0.8; same human-judged quality suite), side-by-side compare, pick the keeper.

**Platform & ops**
- Settings UI: media folders (multiple), durations, transition, story-mode weights, unedited-RAW render-on-upscale, unrenderable-edits fallback (labeled export/preview vs skip), track JPEG exports for cleanup (default on), on-screen status markers (default on — routine chrome only; actionable messages never suppressible), video sound (default muted), close-Lightroom-on-screensaver-start (default off), render look-ahead depth (default 10), navigation-history size (default 200). Persisted via the atomic `<userData>/settings.json` store.
...
Per-image routing to the owning editor (sidecar/namespace detection); `execFile`-based `darktable-cli` and `rawtherapee-cli` wrappers (never shell strings; serialized renders; private darktable configdir); the LR companion plugin (import → forced sidecar read → export, with Afterglow managing LR's lifecycle); just-in-time ephemeral rendering for a moment-aware look-ahead queue under the rendering predicate (sibling JPEGs never a display shortcut when renderable — they become scan-time organizer inventory behind a default-on setting, displayed labeled only in the irreproducible cases; renders keyed by full render identity — RAW mtime/size, sidecar content digests, artifact route, renderer version, output settings — and deleted after leaving recent history); embedded previews shown when adequate, rendered when they would upscale; substitution-or-hold with an honest render-wait status when a render isn't ready; RAW+JPEG pair de-dup (incl. `JPEG\` subfolders); FUSE/network-mount exclusion; flag queue reachable from the settings screen (tester feedback).
...
**v0.7 — Organizer mode**
The flag queue becomes workable and the library becomes organizable, reusing mobile's shipped three-layer state model (`docs/STATE_MODEL.md`: one verdict per photo — unreviewed/kept/culled/trashed — orthogonal actions with a `queued | applied | error` lifecycle, annotations as facts; durable queues; plan/preview → one confirmed batch apply; decisions reversible until commit). Every operation works on the photo's **file family** (RAW + JPEG sibling + every edit sidecar): renames, moves, and trash carry the whole family in lockstep — with one defined exception, the **sibling-only delete** for export-inventory flags (the entry names the export itself; deleting just it is the intent). Byte-identical RAWs whose sidecars differ are surfaced as edit conflicts for review — never auto-trashed as duplicates. **Redundant exports are flagged wherever a better-format original exists** — v0.6 already builds this inventory for RAW siblings at scan time; v0.7 extends it to JPEGs beside HEIC (same-basename pairs are common in phone backups: ~1,500 on one family phone alone) — feeding the same sibling-only delete action, so the library can converge on originals-plus-sidecars rather than accumulating derived copies. Four rungs sharing one plan/apply/verify machinery: exact-duplicate cleanup (size-bucketed SHA-256 over family-aware groups); rename + date normalization (`YYYYMMDD_HHMMSS_<CameraModel>`, camera segment dropped when no camera metadata exists — scans, old JPGs; `_1` collision suffixes; conflicts among strong date signals — EXIF/filename/folder — always queue for human review, mtime being fallback-only; adjudications written back to EXIF via exiftool); folder-layout moves against a per-library configurable template (default `<YYYY>/<YYYYMMDD>`; deliberate non-dated folders — person albums, scans — are legitimate, not violations); burst-culling compare UI over the index, consuming the **shared embedding grouping engine core gains in m0.8** (burst gate → centroid linkage → adjacent-burst merge, quality pinned by the committed human-judged regression suite; dHash survives only as a near-duplicate floor) — desktop's embedding compute path (decode + model runtime) is a v0.7 design-time choice, and desktop builds its own compare model (core's `DeckSession`/`CullSession` are deleted in m0.8; mobile's swipe-deck is the UX reference, not shared code). Queue actions cover the D/E/M/R/N/T flags (OS trash, move, open in editor, rename, date-fix). New shared-core modules: duplicate matching, rename planning, multi-signal date resolution — desktop consumes first, mobile later.
...
- **Lightroom coupling.** The LR tier needs LR installed, running, and a plugin loaded, and unattended operation must stay dialog-free. Both LR prompts are solved via preference keys, but those keys are undocumented Adobe internals an update could rename or reset — onboarding verifies/pre-seeds them on every run, and the failure mode is deferred jobs with an honest status, never a hung show. Mitigation beyond that: the tiered messaging above, in-app labels included.
...
- Desktop embedding runtime for the shared grouping engine (the m0.8 study settled the algorithm and model — MediaPipe MobileNetV3-large embeddings; dHash remains only as a near-duplicate floor): how desktop computes vectors (Node tflite/ONNX runtime vs bundled CLI) is decided at v0.7 design time against the same frozen label suite.
