...
113. **Round-41 refinements**: the window write re-checks the gate after the embed/regroup reads (a user write starting mid-window no longer waits behind it); one gate timeout releases EVERY waiter (stale timers could un-arm a fresh write's priority); the staged-bytes reads share one snapshot (a scan sizing a row between split reads would drop it from both populations). Gate 6 vetted: PROCEED with the UNIVERSAL APK (splits investigation deferred to the backlog; the release notes state the ~163 MB size).

114. **The reclaimable figure stats staged files LIVE** (round 42): the recorded `size_bytes` is only the fallback when a stat fails — a file edited in place after its last scan must not report a stale size. The staged set is user-bounded, so the sweep stays cheap; `size_bytes` remains scan-recorded for future corpus uses. (Round 43 addendum: a successful zero-byte stat is a real value — only FAILED stats use the recorded fallback, via `fileSizeOrNull`.)

115. **Refresh livelock fix (device-caught, post-round-44; hardened through rounds 45–48)**: the v14 S23 validation showed an EMPTY review queue for the entire 27 k warm rescan while the S10e's post-scan queue was fully populated — the round-35 retry-until-latest `refresh()` loops livelocked: concurrent scan-status refreshes (every ~2 s during a scan) mutually superseded each other's generations, so no pass ever committed until the scan settled. Final design: `refresh()` is SINGLE-FLIGHT with PER-PASS settlement — one chain runs passes back-to-back, and every caller holds a pass barrier settled by the first pass that starts at/after its request (commit or deliberate skip; a failing pass rejects only its own waiters), so barrier callers never wait for chain quiescence and cannot starve under sustained scan-status requests. `refreshScoped` pauses the chain at a pass boundary (bounded drain), reinstalls its roots as the fallback after the drain, holds the slot for its strict pass, then on success serves the coalesced barriers per pass and on failure rejects them (running a pass mid-rollback would repaint the rejected source scope). Device-verified on the S23: the queue is live and growing during an active 27 k rescan, and the strictness-apply modal (entry 102's requested test) measured ~0.7 s idle and ~0.5 s mid-scan by screenrecord frame analysis.
